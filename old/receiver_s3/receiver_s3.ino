...
#define AUDIO_LATE_US 4000        // ~3 DMA buffer periods; past this the ring
                                  // has likely drained (audible dropout)
...
typedef struct {
  ...
  // Diagnostics, reset every status print: worst inter-packet gap, count of
  // gaps >100 ms, and count of sudden rpm collapses (>10 rpm -> <1 rpm in
  // one packet). Any of these spiking when Serato flips to INT points at
  // the radio/transmitter side, not the audio side.
  uint16_t maxGapMs;
  uint16_t gapsOver100;
  uint16_t rpmDrops;
  uint8_t  mac[6];
} deck_state;
...
typedef struct {
  ...
  // Diagnostics, reset every status print. One loop iteration normally takes
  // one DMA buffer period (~1.45 ms at 64/44100); the ring holds 4 buffers,
  // so an iteration gap past ~3 periods means the DMA ring likely drained
  // and the DAC emitted silence (tx_desc_auto_clear) = audible dropout.
  volatile uint32_t maxLoopUs;
  volatile uint16_t lateLoops;
} audio_deck_state;
...
void debugPrintStatus() {
  ...
  for (uint8_t i = 0; i < 2; i++) {
    deckStates[i].maxGapMs = 0;
    deckStates[i].gapsOver100 = 0;
    deckStates[i].rpmDrops = 0;
  }
  ...
  for (uint8_t i = 0; i < 2; i++) {
    ...
    uint32_t audioMax = audioDecks[i].maxLoopUs;
    uint16_t audioLate = audioDecks[i].lateLoops;
    audioDecks[i].maxLoopUs = 0;
    audioDecks[i].lateLoops = 0;
    Serial.printf("D%u %s rpm=%7.2f seq=%lu rx=%lu lost=%u age=%lums gapMax=%u gap100=%u rpmDrop=%u aMax=%luus aLate=%u  ",
                  i + 1,
                  online ? "ON " : "OFF",
                  rpm,
...
void updateLed(int onlineCount) {
  if (onlineCount == lastLedCount) return;        // refresh only on change
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  ...
  if (state->lastSeenMillis != 0) {
    uint32_t gap = millis() - state->lastSeenMillis;
    if (gap > state->maxGapMs) state->maxGapMs = (uint16_t)min(gap, 65535UL);
    if (gap > 100) state->gapsOver100++;
  }
  ...
  if (state->seen && abs((int)state->rpmCenti) > 1000 && abs((int)packet.rpmCenti) < 100)
    state->rpmDrops++;
  state->seen = true;
...
void audioTask(void *param) {
  ...
  while (true) {
    ...
    uint32_t dt = nowUs - prevUs;
    prevUs = nowUs;
    if (dt > deck->maxLoopUs) deck->maxLoopUs = dt;
    if (dt > AUDIO_LATE_US) deck->lateLoops++;
  }
}
