...
#define EVT_DRDY_FALLBACK 13     // (BMI160 only) INT1 data-ready unusable
...
// at the commanded base speed is taken as exactly that speed and the gyro
// scale trim is derived from it, then saved to flash. Calibrate at 0% PITCH
// or it will learn the wrong trim. NO auto-arm at boot - portal only.
...
// Real bias is small (MPU6050 spec +-20 dps); a platter is 200 dps at 33 1/3
// and 270 at 45, so 30 dps still separates them cleanly.
...
static bool requireRegisterWrite(uint8_t reg, uint8_t value, const char *name) {
  ...
  Serial.printf("ERR mpu6050 write %s (reg 0x%02X) - check I2C/wiring\n", name, reg);
  return false;
}
...
bool setupMPU6050() {
  ...
  for (int tries = 0; tries < 10; tries++) {
    if (readRegisters(MPU6050_REG_WHO_AM_I, &id, 1) && id == MPU6050_WHO_AM_I_VAL) break;
    delay(20);
  }
  if (id != MPU6050_WHO_AM_I_VAL) {
    Serial.printf("ERR mpu6050_id (got 0x%02X, want 0x%02X) - check I2C/wiring\n",
                  id, MPU6050_WHO_AM_I_VAL);
    imuFaultCode = IMU_FAULT_ID;
    return false;
  }

  // Wake the part and use the X-gyro PLL, the datasheet's preferred clock
  // source for stable timing.
  bool ok = requireRegisterWrite(MPU6050_REG_PWR_MGMT_1, 0x01, "power/clock");
  delay(50);
  ok = requireRegisterWrite(MPU6050_REG_CONFIG, GYRO_DLPF_CFG, "low-pass filter") && ok;
  ok = requireRegisterWrite(MPU6050_REG_SMPLRT_DIV, 0x00, "sample rate") && ok;   // 1 kHz
  ok = requireRegisterWrite(MPU6050_REG_GYRO_CONFIG, GYRO_FS_SEL, "gyro range") && ok;
  delay(50);
...
// trim = target/measured, and persists it.
...
// Reconnect without stopping the 500 Hz data stream. Link recovery remains
// transparent to the receiver's audio/failover path.
...
void setup() {
  ...
  Serial.println();
  Serial.println("BOOT: dvs transmitter MPU6050 alive");
  ...
  bool imuOk = setupMPU6050();
  Serial.println(imuOk ? "BOOT: mpu6050 OK" : "BOOT: mpu6050 FAULT - continuing to report it");
