...
// [TEST] 4-quadrant touch cues from a DECK unit (the S3-display puck),
// mapped to MIDI note = DECK_CUE_BASE + (deck-1)*4 + quadrant (60-63 deck 1,
// 64-67 deck 2). Distinct from the dicer's 36-53.
#define DECK_CUE_BASE 60
volatile uint16_t deckCueMask[2] = { 0, 0 };
volatile bool     deckCueSeen[2] = { false, false };
uint16_t deckCuePrev[2] = { 0, 0 };
uint8_t  deckCueNote[2][4] = { { 0, 0, 0, 0 }, { 0, 0, 0, 0 } };
...
// consistent.
// Diff each deck unit's 4-quadrant touch cues into MIDI notes. Same
// state-based scheme as the dicer; a lost packet only delays an edge.
void serviceDeckCueMidi() {
  for (uint8_t d = 0; d < 2; d++) {
    uint16_t mask; bool seen;
    portENTER_CRITICAL(&stateMux);
    seen = deckCueSeen[d]; mask = deckCueMask[d];
    portEXIT_CRITICAL(&stateMux);
    if (!seen) continue;
    uint16_t changed = mask ^ deckCuePrev[d];
    if (!changed) continue;
    for (uint8_t q = 0; q < 4; q++) {
      if (!(changed & (1 << q))) continue;
      if (mask & (1 << q)) {
        uint8_t note = DECK_CUE_BASE + d * 4 + q;
        deckCueNote[d][q] = note;
#if USB_MIDI_ACTIVE
        MIDI.noteOn(note, 127, MIDI_CHANNEL);
#endif
#if DEBUG_SERIAL
        Serial.printf("CUE deck %u q%u DOWN -> note %u on\n", d + 1, q + 1, note);
#endif
      } else if (deckCueNote[d][q]) {
#if USB_MIDI_ACTIVE
        MIDI.noteOff(deckCueNote[d][q], 0, MIDI_CHANNEL);
#endif
#if DEBUG_SERIAL
        Serial.printf("CUE deck %u q%u up -> note %u off\n", d + 1, q + 1, deckCueNote[d][q]);
#endif
        deckCueNote[d][q] = 0;
      }
    }
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  ...
  if (packet.msgType == MSG_BUTTONS) {          // deck's 4-quadrant touch cues
    portENTER_CRITICAL_ISR(&stateMux);
    deckCueMask[packet.deckId - 1] = (uint16_t)packet.rpmCenti;
    deckCueSeen[packet.deckId - 1] = true;
    portEXIT_CRITICAL_ISR(&stateMux);
    return;
  }
